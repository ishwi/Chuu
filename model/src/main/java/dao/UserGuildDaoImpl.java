...
public class UserGuildDaoImpl implements UserGuildDao {
    ...
    @Override
    public void unflagAsBotted(String lastfmId, Connection connection) {
        String queryString = "Delete from botted where lastfm_id = ?";
        ...
        queryString = "update user set botted_account  =  false where lastfm_id = ?";
        try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)) {
            preparedStatement.setString(1, lastfmId);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
