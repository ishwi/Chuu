...
public class TestResources extends ExternalResource {
    ...
    public static void deleteCommonArtists() {
        dao.insertNewUser(new LastFMData("guilleecs", ogJDA.getSelfUser().getIdLong(), channelWorker
                .getGuild().getIdLong(), setUp, true, WhoKnowsMode.IMAGE, ChartMode.IMAGE, RemainingImagesMode.IMAGE, 5, 5, null, true, true, showBotted, TimeZone.getDefault(), token, session));
    ...
    public static void insertOnlyKnownSecond(String second, int plays) {
        dao.insertNewUser(new LastFMData("guilleecs", ogJDA.getSelfUser().getIdLong(), channelWorker
                .getGuild().getIdLong(), setUp, true, WhoKnowsMode.IMAGE, ChartMode.IMAGE, RemainingImagesMode.IMAGE, 5, 5, PrivacyMode.NORMAL, true, true, showBotted, TimeZone.getDefault(), token, session));
        ArrayList<ScrobbledArtist> scrobbledArtistData = new ArrayList<>();
