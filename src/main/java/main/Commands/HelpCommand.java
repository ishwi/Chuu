...
public class HelpCommand extends MyCommand {
	...
	private final TreeMap<String, Map<String, MyCommand>> commands;
	...
	public MyCommand registerCommand(MyCommand command) {
		String parserName = command.parser.getClass().getName();
		Map<String, MyCommand> possibleNewMap = new HashMap<>();
		Map<String, MyCommand> previousTreeMap = commands.putIfAbsent(parserName, possibleNewMap);
		if (previousTreeMap == null) {
			possibleNewMap.put(command.getName(), command);
		} else {
			previousTreeMap.put(command.getName(), command);
			possibleNewMap = null;
		}
		return command;
	}
	...
	public void sendPrivate(MessageChannel channel, String[] args) {
		if (args.length < 2) {
			StringBuilder s = new StringBuilder();

			Set<Map.Entry<String, Map<String, MyCommand>>> entries = commands.entrySet();
			for (Map.Entry<String, Map<String, MyCommand>> entry : entries) {
				s.append("Parser ").append(entry.getKey()).append(" usage\n");
				for (MyCommand c : entry.getValue().values()) {
