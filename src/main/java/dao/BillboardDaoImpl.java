...
public class BillboardDaoImpl implements BillboardDao {
    ...
    public Week getCurrentWeekId(Connection connection) {
        @Language("MariaDB") String queryString = "SELECT id,DATEDIFF(week_start,?),week_start as ord  from week order by  ord limit 1";
        try (PreparedStatement preparedStatement = connection.prepareStatement(queryString)) {
            ...
            Date week_start = resultSet.getDate("ord");
    ...
    public List<BillboardEntity> getBillboard(Connection connection, int week_id, long idLong, boolean doListeners) {
        ...
        @Language("MariaDB") String queryString =
                "SELECT \n" +
                        ...
                        "(select max(position) from " + table + " t where t.artist_id = a.artist_id and t.guild_id = a.guild_id and t.track_name = a.track_name ) as peak,\n" +
    ...
    public void insertUserData(Connection connection, List<TrackWithArtistId> trackList, String lastfmId, int weekId) {
        ...
        StringBuilder mySql =
                new StringBuilder("INSERT INTO  user_billboard_data" +
                        "                  (week_id,artist_id,track_name,scrobble_count,lastfm_id) VALUES (?,?,?,?,?) ");
        ...
        try {
            ...
            for (int i = 0; i < trackList.size(); i++) {
                ...
                preparedStatement.setLong(5 * i + 1, weekId);

                preparedStatement.setLong(5 * i + 2, trackWithArtistId.getArtistId());
                preparedStatement.setString(5 * i + 3, trackWithArtistId.getName());
                preparedStatement.setInt(5 * i + 4, trackWithArtistId.getPlays());
                preparedStatement.setString(5 * i + 5, lastfmId);
