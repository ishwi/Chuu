...
public record TrackExplanation(Explanation artist, Explanation song) {
    ...
    public TrackExplanation() {
        this(
                ...
                () -> new ExplanationLineTypeAutocomplete(NAME, "The song of the artist to query for, separated with -", OptionType.STRING, TrackExplanation::searchChoices));
    }
        ...
        if (opt.isEmpty()) {
            ...
            try {
                ...
                if (StringUtils.isBlank(search)) {
                    return db.getGlboalTopArtistTracks(artistId, 25).stream().map(x -> Autocompletable.of(x.getAlbum())).toList();
                } else {
                    return db.searchTracksForArtist(search, artistId, 25).stream().map(Autocompletable::of).toList();
                }
    ...
    private static List<Command.Choice> nonExistingArtist(String search, ChuuService db, CommandAutoCompleteInteractionEvent e) {
        if (StringUtils.isBlank(search)) {
            return getUserTopTracks(db, e);
        } else {
            return db.searchTracks(search, 25).stream().map(Autocompletable::of).toList();
        }
    }


    private static List<Command.Choice> getUserTopTracks(ChuuService db, CommandAutoCompleteInteractionEvent e) {
        try {
            LastFMData data = db.findLastFMData(e.getUser().getIdLong());
            return db.getTopTracks(data.getName(), 25).stream().map(x -> Autocompletable.of(x.getName())).toList();
        } catch (InstanceNotFoundException ex) {
