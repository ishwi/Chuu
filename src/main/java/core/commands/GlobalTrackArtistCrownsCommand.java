...
public class GlobalTrackArtistCrownsCommand extends ConcurrentCommand<NumberParameters<ArtistParameters>> {
    ...
    public UniqueWrapper<TrackPlays> getList(NumberParameters<ArtistParameters> params) {
        ...
        return getService().getArtistGlobalTrackCrowns(params.getInnerParams().getLastFMData().getName(), params.getInnerParams().getScrobbledArtist().getArtistId(), Math.toIntExact(threshold), !params.hasOptional("nobotted"));
    }
    ...
    @Override
    public void onCommand(MessageReceivedEvent e, @NotNull NumberParameters<ArtistParameters> params) throws LastFmException, InstanceNotFoundException {
