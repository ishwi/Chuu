...
public class TotalTrackNumberCommand extends ConcurrentCommand<NumberParameters<ChuuDataParams>> {
    public TotalTrackNumberCommand(ServiceView dao) {
        super(dao);
    }
    ...
    public String getDescription() {
        return ("Number of songs listened by an user");
    }

    @Override
    public List<String> getAliases() {
        return List.of("songs", "trackcount");
    }
    ...
    protected void onCommand(Context e, @NotNull NumberParameters<ChuuDataParams> params) {
        ...
        int plays = db.getUserTrackCount(lastFmName, threshold == 0 ? -1 : threshold);
        String filler = "";
        if (threshold != 0) {
            filler += " with more than " + threshold + " plays";
        }
        sendMessageQueue(e, String.format("**%s** has scrobbled **%d** different %s%s", username, plays, CommandUtil.singlePlural(plays, "song", "songs"), filler));
